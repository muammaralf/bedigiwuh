...
import { IEventUseCase } from "../../service/event/event.use_case";
...
export class EventController implements BaseController {
  ...
  init(): void {
    ...
    this.app.post(`/${this.prefix}`, async (req: Request, res: Response) => {
      try {
        await this.service.create(objectToSnake(req.body) as Event)
    ...
    this.app.put(`/${this.prefix}/:id`, async (req: Request, res: Response) => {
      try {
        await this.service.update(parseInt(req.params.id), objectToSnake(req.body) as Event)
        res.status(200).json(dataToRestResponse(null))
      } catch (error) {
        res.status(400).json(errorToRestResponse(error))
      }
